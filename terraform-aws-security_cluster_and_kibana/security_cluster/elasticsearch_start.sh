...
# Function to wait until the specific node certificates are ready
wait_for_node_certs() {       
    while true; do
        date
...
generate_certs() {
    if [ "$ES_NODE_NAME" = "$ES_FIRST_NODE" ]; then
        date
        ...
        for i in "${!NAMES[@]}"; do
            ...
            # Determine the service based on the node name
            if [[ "$NAME" =~ ^master- ]]; then
                SERVICE="$ES_MASTER_SERVICE"
            elif [[ "$NAME" =~ ^data- ]]; then
                SERVICE="$ES_DATA_SERVICE"
            fi
            
            # Generate full DNS
            DNS="es-${NAME}.${SERVICE}.${ES_NAMESPACE}.svc.cluster.local"
